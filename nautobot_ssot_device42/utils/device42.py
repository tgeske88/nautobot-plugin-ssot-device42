...
class Device42API:  # pylint: disable=too-many-public-methods
    ...
    def get_patch_panels(self) -> List[dict]:
        ...
        """
        query = "SELECT a.name, a.serial_no, a.customer_fk, a.building_fk, a.room_fk, a.size, m.number_of_ports, m.name as model_name, m.port_type_name as port_type, v.name as vendor, r.name as rack, a.start_at as position FROM view_asset_v1 a LEFT JOIN view_patchpanelmodel_v1 m ON m.patchpanelmodel_pk = a.patchpanelmodel_fk JOIN view_vendor_v1 v ON v.vendor_pk = m.vendor_fk JOIN view_rack_v1 r ON a.rack_fk = r.rack_pk WHERE a.patchpanelmodel_fk is not null"
        return self.doql_query(query=query)
